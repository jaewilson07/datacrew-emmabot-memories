...
- **Cloud Amplifier schema changes**: new columns auto-sync on next DataSet refresh (Cloud Integration service manages schemas; Schema Management doesn't apply) — manual work is downstream: ETL input tiles, cards, service-account grants → `knowledge-base/domo/gotchas/cloud-amplifier-schema-changes.md`
